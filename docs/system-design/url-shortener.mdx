...
Line-by-line redirect reading:

```http
HTTP/1.1 302 Found
Location: https://merchant.example/sale
Cache-Control: no-store
```

- `302 Found` means the redirect target can change later. Use this when you need analytics, abuse blocking, expiration, or mutable destinations.
- `Location` is the real destination. Validate it during short-link creation so your service does not become an open redirect platform.
- `Cache-Control: no-store` tells browsers and intermediaries not to cache the redirect. Without it, a corrected malicious or expired destination can remain stuck in user agents.

```http
HTTP/1.1 301 Moved Permanently
Location: https://docs.example/backend
Cache-Control: public, max-age=31536000
```

- `301` can be cached aggressively by browsers and CDNs. That is fast, but dangerous if owners can edit destinations.
- `max-age=31536000` trades control for performance. Only use it when the mapping is immutable.

Proof drill: create a link, fetch it once, change the destination, then fetch again through a browser or CDN. If you used `301`, the client may never ask your service again.
