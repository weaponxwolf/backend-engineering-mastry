...
### The Pigeonhole Principle for Strong Consistency
$$W + R > N$$

If $N = 3$, and we configure **$W = 2$ and $R = 2$**:
$$W + R = 4 > 3$$
By the **Pigeonhole Principle**, the set of nodes written to ($W$) and the set of nodes read from ($R$) **must overlap by at least one node**. The overlapping node is guaranteed to hold the latest version!
...
| **Strong Consistency (CP)**| $W=2, R=2, N=3$ | Fast ($\le 8\text{ms}$) | Fast ($\le 8\text{ms}$) | **Strong Consistency** (Overlapping quorum) | User account profiles, order states |
| **Fast Writes / Heavy Reads**| $W=1, R=3, N=3$ | Slower (Wait for all 3) | Instant (Wait for 1) | **Strong Consistency** ($1 + 3 > 3$) | High-frequency sensor logging |
...
To achieve **$99.999\%$ Write Availability**, Dynamo implements **Sloppy Quorums with Hinted Handoff**:
...
<AccordionGroup>
  ...
  <Accordion title="2. How does the Pigeonhole Principle guarantee Strong Consistency in Quorum Replication?">
    If $N$ is the replication factor, $W$ is the write quorum, and $R$ is the read quorum, configuring:
    $$W + R > N$$
    guarantees strong consistency.
    
    By the **Pigeonhole Principle**, if you write to $W$ nodes and read from $R$ nodes out of a total of $N$ nodes, the write set and read set **must overlap by at least one node**. That overlapping node holds the most recent write version. When the read coordinator evaluates responses, it selects the highest version, guaranteeing that stale data is never returned.
  </Accordion>
