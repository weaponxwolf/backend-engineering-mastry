...
Line-by-line vector clock reading:

- `clockMap` stores one logical counter per node or region that has written this key.
- `increment(nodeId)` is called only by the node accepting a new write. It copies the old map, increments that node's counter, and returns a new clock so previous versions remain inspectable.
- `getOrDefault(node, 0L)` treats missing nodes as "this version has seen zero writes from that node."
- `hasGreater` means this clock has at least one node counter ahead of the other version.
- `hasLesser` means this clock is behind in at least one node counter.
- `hasGreater && hasLesser` means neither version causally contains the other. They are concurrent siblings, not older/newer versions.
- `merge(...)` takes pairwise maximums after the application resolves sibling values. The merged clock records that the new value has observed both histories.

```json
{
  "cartId": "cart-42",
  "items": ["sku-1", "sku-9"],
  "vectorClock": {"us-east-1": 7, "eu-west-1": 3}
}
```

Proof drill: write `cart-42` in two isolated regions during a network partition, heal the partition, and verify the store returns siblings instead of blindly overwriting one customer's item.
...
## Distributed KV Store Fundamentals Drill

A distributed key-value store looks simple because the API is small: `put(key, value)` and `get(key)`. The complexity hides underneath: where the key lives, how many copies exist, what happens when nodes disagree, and how the system repairs itself.

```mermaid
flowchart LR
    Client --> Router[Coordinator / router]
    Router --> Ring[Partition map]
    Ring --> N1[Replica 1]
    Ring --> N2[Replica 2]
    Ring --> N3[Replica 3]
    N1 --> Disk1[Commit log + SSTables]
    N2 --> Disk2[Commit log + SSTables]
    N3 --> Disk3[Commit log + SSTables]
...
| Axis | Question | Typical choices |
| --- | --- | --- |
| Partitioning | Which node owns a key? | hash ranges, consistent hashing, virtual nodes |
| Replication | How many copies exist? | replication factor 3, multi-AZ, multi-region |
| Consistency | How many replicas must agree? | `R=1/W=1`, quorum, leader-based writes |
| Storage engine | How is data stored on disk? | LSM tree, B-Tree, append-only log |
| Repair | How do replicas converge? | read repair, background repair, hinted handoff |
| Rebalancing | What happens when nodes join or leave? | token movement, streaming, throttling |

### Example: Shopping Cart KV Store

Requirement:

```markdown
Users should rarely lose cart updates.
Reads should be low latency.
It is acceptable for another device to see the update after a short delay.
```
...
```markdown
Key: cart:{userId}
Value: JSON or encoded cart snapshot
Replication factor: 3
Write policy: W=2
Read policy: R=1 with read repair
Conflict handling: version vector or last-write-wins only if product accepts overwrite risk
```

The senior detail is conflict handling. If two devices update the cart during a partition, "last write wins" can drop one item. A deeper design stores item-level operations or merges carts by line item instead of replacing the whole snapshot blindly.

### Common Interview Traps

| Trap | Why it is shallow | Better answer |
| --- | --- | --- |
| "Just hash the key." | Node changes remap too many keys. | Use consistent hashing or partition maps. |
| "Replicate to 3 nodes." | Replication alone does not define read/write correctness. | Specify `R`, `W`, failure behavior, and repair. |
| "Use eventual consistency." | Too vague. | State what stale result can occur and why it is acceptable. |
| "Store JSON." | Large values make partial updates and conflicts harder. | Choose value shape based on write pattern. |
...
```markdown
Implement an in-memory KV store with 3 replicas per key and a simple hash router.
```

Break:

```markdown
Kill one replica during writes.
Add a new node and observe key remapping.
Perform two conflicting writes to the same key.
```
...
Add background repair for stale replicas.
```
