...
```text
Example:
If 5% of your checkout request is strictly serial (acquiring an exclusive database row lock):
Even with INFINITE servers (N -> ∞):
Max Theoretical Speedup = 1 / (0.05 + 0) = 20x speedup!
Adding more servers beyond 20 nodes yields ZERO additional performance!
```
