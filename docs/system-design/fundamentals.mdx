...
### Line-by-Line Design Reading: Consistent Hash Lookup

When you present consistent hashing, do not only draw the ring. Explain the lookup line by line:

```java
long keyHash = murmur3_128(key);
Map.Entry<Long, Server> entry = ring.ceilingEntry(keyHash);
Server owner = entry != null ? entry.getValue() : ring.firstEntry().getValue();
```

- `murmur3_128(key)` turns an uneven business key like `user:42` into a uniformly distributed number. Without hashing, numeric or alphabetical patterns can overload one shard.
- `ring.ceilingEntry(keyHash)` asks: "Which virtual node is the first clockwise owner at or after this key position?"
- `entry != null ? ... : ...` handles ring wraparound. If the key lands after the largest vnode, ownership wraps to the first vnode at the beginning of the ring.
- `entry.getValue()` returns the physical server behind that virtual node. Many virtual nodes point to the same physical server to smooth distribution.

Proof drill: build a ring with 3 servers and 200 virtual nodes each, hash 1,000,000 keys, then remove one server. A healthy implementation moves roughly one-third of keys, not all keys.

---

## System Design Fundamentals Interview Map

System design is not drawing boxes. It is making trade-offs explicit under constraints.

### 1. The required interview flow

```mermaid
flowchart TD
    Req["Clarify requirements"] --> Capacity["Estimate capacity"]
    Capacity --> API["Design APIs"]
    API --> Data["Design data model"]
    Data --> HLD["High-level architecture"]
    HLD --> Deep["Deep dives"]
    Deep --> Failure["Failure modes"]
    Failure --> Tradeoffs["Trade-offs and alternatives"]
```

Skipping requirements and jumping into architecture is a common junior mistake.
...
### 3. Every design needs a data model

If you cannot name the tables, indexes, keys, and ownership boundaries, the design is still vague.

### 4. Every design needs failure modes

Ask:

- What happens if a database is slow?
- What happens if Kafka is down?
- What happens if a region fails?
- What happens if a client retries?
- What happens if one key becomes hot?
...
<Steps>
  <Step title="Build">
    Design a URL shortener with API, schema, cache, and redirect flow.
  </Step>
  <Step title="Break">
    Add duplicate alias creation, hot celebrity links, and cache node failure.
  </Step>
  <Step title="Observe">
    Identify uniqueness races, hot partitions, and cache avalanche behavior.
  </Step>
  <Step title="Fix">
    Add unique constraints, consistent hashing, hot-key salting, and backpressure.
  </Step>
  <Step title="Prove">
    Explain tests, metrics, and runbooks that would validate the design.
  </Step>
</Steps>
