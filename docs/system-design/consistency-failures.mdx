...
| System | Partition Behavior (P/A or P/C) | Normal Behavior (E/L or E/C) |
| :--- | :--- | :--- |
| **PostgreSQL / MySQL** (Sync Replication) | **PC**: Blocks writes if sync replica is isolated | **EC**: Waits for replica ACK before returning |
| **PostgreSQL / MySQL** (Async Replication) | **PA**: Primary accepts writes even if replicas unreachable | **EL**: Returns immediately after local WAL commit |
| **Apache Cassandra / DynamoDB** | **PA**: All nodes accept writes (hinted handoff) | **EL**: Reads/writes complete at $W=1, R=1$ |
| **MongoDB** (Majority Write Concern) | **PC**: Rejects writes if primary cannot reach majority | **EC**: Waits for majority acknowledgment |
| **Google Spanner / CockroachDB** | **PC**: Enforces Raft/Paxos consensus; halts partitions | **EC**: TrueTime / Raft consensus before returning |
| **Yahoo PNUTS / Couchbase** | **PC**: Blocks writes on partition split | **EL**: Serves reads locally with minimal latency |
...
For a fixed set of $N$ replicas, the following inequality makes every read set intersect every acknowledged write set:
...
> [!NOTE]
> **Pigeonhole Intersection Proof**:
> For $N = 5$ nodes with $W = 3$ and $R = 3$:
> $$R + W = 3 + 3 = 6 > 5$$
> By the Pigeonhole Principle, any subset of 3 read replicas must contain at least one replica from the 3 write nodes that acknowledged the write. Version resolution is a separate protocol.

<Warning>
**Overlap is not a complete proof of linearizability.** The replicas must belong to the same set and retain the acknowledged value. Concurrent writes, incomplete writes, clock skew, and membership changes need defined behavior. Comparing wall-clock timestamps does not establish real-time order. Sloppy quorums can use substitute nodes and lose the intersection assumed by the formula.

For example, a partial write can reach one replica and time out. One read may see it; a later read from a different pair may miss it. A protocol needs additional steps to prevent that result. Cassandra uses Paxos-based lightweight transactions for linearizable conditional operations. See [Cassandra's guarantees](https://cassandra.apache.org/doc/stable/cassandra/architecture/guarantees.html) and [Dynamo's sloppy quorum design](https://www.allthingsdistributed.com/2007/10/amazons_dynamo.html).
</Warning>

### Tunable Consistency Topologies

<Tabs>
  <Tab title="Balanced overlap (W=Quorum, R=Quorum)">
    **Topology Formula**: $N = 3, W = 2, R = 2 \implies R + W = 4 > 3$

    - **Pros**: Both reads and writes can complete if any two of the fixed three replicas are reachable.
    - **Performance**: Balanced read and write latencies.
    - **Use Case**: Data requiring visibility of acknowledged writes under the store’s documented resolution rules. Financial invariants need an appropriate transaction or conditional-write protocol.
  </Tab>
  <Tab title="Fast Writes, Heavy Reads (W=1, R=N)">
    **Topology Formula**: $N = 3, W = 1, R = 3 \implies R + W = 4 > 3$
  ...
  <Tab title="Fast Reads, Heavy Writes (W=N, R=1)">
    **Topology Formula**: $N = 3, W = 3, R = 1 \implies R + W = 4 > 3$
  ...
  <Tab title="Stale Reads / High Availability (R + W <= N)">
    **Topology Formula**: $N = 3, W = 1, R = 1 \implies R + W = 2 \le 3$

    - **Pros**: Maximum possible availability and lowest latency. Both reads and writes survive multiple node failures.
    - **Cons**: A read can miss an acknowledged write. Concurrent read-modify-write operations can lose updates; SQL isolation anomaly names do not directly describe every key-value store.
...
Networks can lose messages. Applications and brokers can provide at-least-once processing through retained data and retries, under stated availability and retention assumptions. Therefore, any mutating endpoint (charges, orders, balance transfers) must enforce idempotency using unique request tokens.
...
#### Line-by-line idempotency reading

- `idempotency_key` is supplied by the client for one logical operation, such as "create payment attempt." Reusing it for a different operation is a client bug.
- `user_id` prevents one user's key from accidentally or maliciously colliding with another user's request.
- `request_path` prevents the same key from being replayed against a different endpoint.
- `request_hash` protects against accidental key reuse with a different payload. The server can reject "same key, different body" instead of returning a misleading cached response.
- `status` tells whether the first request is still `IN_PROGRESS`, already `COMPLETED`, or failed in a known way.
- `response_body` lets safe retries return the exact same result after success.
- `expires_at` supports cleanup. Idempotency records are not kept forever unless legal or audit rules require it.

```http
POST /payments HTTP/1.1
Idempotency-Key: pay_2026_09_12_order_8842
Content-Type: application/json

{"orderId":"8842","amountCents":129900}
```

Proof drill: send the same request twice while forcing the first HTTP response to time out after the database commits. The second request must return the original success, not charge the customer again.
...
> 1. **Zero Overselling**: Inventory MUST NEVER oversell (exactly 10,000 units allocated).
> 2. **Downstream Throttling**: Payment provider has a hard ceiling of 500 requests/second and a 2% failure rate.
> 3. **High Availability**: The system must survive the loss of any single database node or message broker.
...
## Consistency Fundamentals Drill

Consistency questions are really questions about what a user is allowed to observe after a write. A backend engineer must be able to say which stale result is acceptable, which stale result is dangerous, and which operations require a stronger contract.
...
sequenceDiagram
    ...
    Client->>Primary: Write order status = PAID
    Primary-->>Client: 200 OK
    Primary-->>ReplicaA: Replicate after 40 ms
    Primary-->>ReplicaB: Replicate after 900 ms
    Client->>ReplicaB: Read order status
    ReplicaB-->>Client: Old status = PENDING
```

This is the core question: is that old `PENDING` response a harmless delay, a broken promise, or a financial bug?

### Consistency Levels In Plain Backend Terms

| Model | What the user can observe | Good for | Dangerous for |
| --- | --- | --- | --- |
| Strong / linearizable | After a successful write, all later reads see it. | balances, locks, inventory reservations | high-latency global reads |
| Read-your-writes | The writer sees their own latest write. | profile edits, settings, shopping cart | shared collaborative state |
| Monotonic reads | One user does not go backward in time. | feeds, dashboards | workflows requiring exact status |
| Eventual consistency | Replicas converge later. | likes, counters, analytics | money movement, authorization |

### The 5 Consistency Questions Interviewers Expect

1. Which operations are correctness-critical?
2. Can users tolerate stale reads? For how long?
3. Must all users see the same order of events?
4. What happens during a network partition?
5. How do retries avoid creating duplicate effects?

### Example: Food Delivery Order Status

The same application can need multiple consistency models:

| Feature | Required behavior | Possible design |
| --- | --- | --- |
| Payment capture | Never double-charge. | Idempotency key plus strongly consistent ledger write |
| Restaurant acceptance | Driver and customer should not see impossible states. | Single order aggregate owner or transactional state machine |
| ETA display | A few seconds stale is acceptable. | Eventually consistent read projection |
| Analytics dashboard | Minutes stale is acceptable. | Async event pipeline |

### Common Trap: Retrying Without Idempotency
...
Server stores key and result in the same transaction as the payment decision
Retry returns the original result instead of creating a second charge
```

### Senior Answer Template

When answering consistency questions, use this exact order:

1. Name the invariant.
2. Decide which operations require strong consistency.
3. Allow weaker consistency only for views that can be repaired.
4. Add idempotency for every retryable command.
5. Add reconciliation for anything asynchronous.
6. State what users see during failover or replica lag.

### Build-Break-Fix Drill

Build a simple order service where `POST /orders/{id}/pay` writes a payment record and emits an event.

Break it:

```markdown
Crash after payment insert but before event publish.
Retry the same request twice.
Read from a lagging replica immediately after payment.
```
...
Route correctness-critical reads to the primary.
Add a reconciliation job that compares ledger rows with emitted events.
```
...
<AccordionGroup>
  ...
  <Accordion title="3. What does R + W > N prove, and what does it leave unproven?">
    It proves that read and write sets overlap when they are subsets of the same fixed $N$ replicas. It does not by itself prove linearizability, atomic read-modify-write, or multi-key transactions. A complete protocol must handle concurrent and incomplete writes, replica failures, version ordering, and reconfiguration. A sloppy quorum may use nodes outside the original set, so the simple intersection argument no longer applies.
  </Accordion>
