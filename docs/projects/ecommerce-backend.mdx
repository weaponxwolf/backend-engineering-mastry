...
Line-by-line inventory proof:

- `@Modifying` tells Spring Data this JPQL changes rows. Without it, Spring treats the query like a read query and fails.
- `UPDATE Inventory i` targets the stock row directly in the database.
- `SET i.stockQuantity = i.stockQuantity - :quantity` performs the decrement inside PostgreSQL while the row lock is held.
- `WHERE i.productId = :productId` scopes the mutation to one product.
- `AND i.stockQuantity >= :quantity` is the invariant. It prevents negative stock even when 500 requests arrive together.
- The returned `int` is the number of rows changed. `1` means reserved; `0` means out of stock or missing product.

Proof drill: seed stock `1`, fire 100 parallel checkout attempts, then assert exactly one returns success and final stock is `0`.
...
The e-commerce backend is a correctness project. It must prove that checkout preserves inventory, money, order history, and idempotency even when users retry, webhooks repeat, and concurrent buyers race for the same stock.
...
sequenceDiagram
    ...
    API->>DB: Create order and price snapshots
    API->>Outbox: Store payment/order event in same transaction
    API-->>Buyer: 201 Created
    Worker->>Outbox: Publish after commit
```

| Proof target | What to demonstrate | Evidence |
| --- | --- | --- |
| No oversell | 10 buyers racing for 2 items gives exactly 2 successes. | Concurrent integration test |
| Idempotent checkout | Retry does not create a second order. | Unique idempotency key per user |
| Price history | Past orders do not change when product price changes. | Snapshot assertion |
| Webhook safety | Same payment webhook can arrive twice. | Deduplication by provider event ID |
| Outbox durability | Order and event commit together. | Transactional test |

Failure drill:

| Failure | Expected protection |
| --- | --- |
| Application crashes after order insert | Outbox row remains available for replay |
| Payment provider times out | Idempotency key prevents duplicate order effects |
| Two users buy last unit | Atomic SQL rejects one user without negative inventory |
| Catalog price changes after checkout | Order item keeps original price |
