...
## Line-by-Line Runtime Proof

Read the create request like this:
...
- `curl -i` keeps headers visible. For create endpoints, headers are part of the proof.
- `POST /api/todos` enters the controller method responsible for creation, not update.
- `Content-Type: application/json` activates JSON deserialization into the request DTO.
- `title` is user input. It must pass DTO validation before an entity is created.

Expected proof points:

- Status is `201 Created`, not `200 OK`.
- The response body contains a generated `id`.
- The `completed` field starts as `false`.
- A later `GET /api/todos/{id}` returns the same row from PostgreSQL.
- A blank title returns `400` and does not insert a row.
