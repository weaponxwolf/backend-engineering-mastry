...
| **Write-Through** | Read Cache $\rightarrow$ On miss, read DB | Write Cache $\rightarrow$ Cache synchronously writes DB | Cache is never stale; read hits are guaranteed | High write latency (waits for both cache and DB writes) |
...
### The Golden Rule: Invalidate (`DEL`), Never Update (`SET`)
...
sequenceDiagram
    ...
    Note over DB,Cache: DISASTER: DB holds 300, but Cache permanently holds stale 200!
```

**The Solution**: Always **delete (evict)** the cache entry upon database mutation. If concurrent writes interleave, the next read will simply trigger a clean cache miss and fetch the latest committed database state.
...
- **L1 Cache (Caffeine)**: Stored in JVM heap. Access time: **under 50 nanoseconds** ($O(1)$ RAM lookup, zero network hops).
- **L2 Cache (Redis)**: Shared distributed cache across all microservice instances. Access time: **1 millisecond**.
...
<AccordionGroup>
  ...
  <Accordion title="2. Why should you delete (evict) a cache key on database update instead of updating the cached value?">
    ...
    If Thread 1 updates the database to $V_1$ and Thread 2 updates the database to $V_2$, network jitter can cause Thread 2's cache write to land before Thread 1's cache write. The cache ends up storing $V_1$ while the database holds $V_2$, creating permanent data corruption. Deleting the key ensures that subsequent reads fetch the authoritative committed state from the database.
  </Accordion>
