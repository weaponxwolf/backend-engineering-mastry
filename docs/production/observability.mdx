...
- **Tail-Based Sampling**: All spans are collected and temporarily buffered in memory by an OpenTelemetry Collector until the entire trace completes. The collector evaluates the trace: if any span contains an error status code (`status >= 500`) or duration exceeds a threshold (e.g., `duration > 1s`), the trace is retained; otherwise, it is dropped. Tail sampling guarantees 100% capture of production incidents while conserving long-term storage.
