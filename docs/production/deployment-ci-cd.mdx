...
<AccordionGroup>
  ...
  <Accordion title="2. Why should database migrations run in a pre-deployment Kubernetes Job instead of Spring Boot startup?">
    ...
    Furthermore, if a migration fails, the pod crashes after holding startup resources. A dedicated migration Job separates schema changes from application startup, but a Kubernetes Job may start the same program more than once. Retain Flyway locking and schema history, keep migrations restart-safe, and make the pipeline wait for successful Job completion before deploying application pods. Abort the rollout on failure. See [Kubernetes Job execution semantics](https://kubernetes.io/docs/concepts/workloads/controllers/job/).
  </Accordion>
  ...
  <Accordion title="5. Why should container images be tagged with Git commit SHAs rather than :latest?">
    ...
    A Git SHA tag helps trace an image to source, but tags can still be overwritten unless the registry enforces immutability. Deploy by image digest to identify the exact artifact. Reproducible builds also require controlled dependencies, base images, and build inputs. See [Kubernetes image tags and digests](https://kubernetes.io/docs/concepts/containers/images/).
  </Accordion>
</AccordionGroup>
