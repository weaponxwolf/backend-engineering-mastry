...
- `pid` is the backend process ID you may later cancel or terminate.
- `now() - query_start AS duration` shows how long the statement has been running.
- `state` distinguishes active work from idle-in-transaction sessions.
- `wait_event_type` and `wait_event` show whether the backend is waiting on locks, I/O, client input, or another resource.
- `query` shows the SQL text. Treat it as sensitive because it may include literals.
- `state != 'idle'` filters out harmless idle connections.
- `pid != pg_backend_pid()` avoids listing your own diagnostic session.
- `ORDER BY duration DESC` puts the most suspicious long-running queries first.

Runbook rule: do not terminate the longest query blindly. First check whether it is the root blocker, a victim waiting behind another transaction, or a legitimate migration.
...
- `pg_cancel_backend(pid)` asks PostgreSQL to cancel the current query while keeping the connection alive. Use this first when possible.
- `pg_terminate_backend(pid)` kills the whole backend connection. It rolls back the active transaction and can disrupt an application request.
- Never terminate autovacuum, replication, or migration sessions casually. Identify the owner and blast radius first.

Proof drill:

```console
In a staging database, open one transaction that locks a row.
Open a second session blocked on that row.
Use pg_blocking_pids to identify the blocker.
Cancel or terminate the blocker.
Verify the waiting session proceeds or fails as expected.
...
## Incident Response Fundamentals Interview Map

Incident response is an engineering system for restoring service under pressure.

### 1. Mitigate first, debug later

During a live outage, the first goal is reducing customer impact:

- Roll back a bad deploy.
- Disable a broken integration.
- Shed non-critical traffic.
- Scale a saturated worker.
- Kill a known blocking query.

Root cause analysis happens after the service is stable.

### 2. Severity levels

| Severity | Meaning |
| :--- | :--- |
| SEV-1 | Major customer-facing outage or data loss risk |
| SEV-2 | Serious degradation with workaround or partial impact |
| SEV-3 | Limited impact, non-critical workflow affected |
| SEV-4 | Minor issue, no urgent customer impact |
...
| Tech Lead | Investigates and executes technical mitigation |
...
assign IC, tech lead, comms, scribe
check recent deploys
check user pain metric
...
<Steps>
  <Step title="Build">
    Create a service with dashboard panels for p99 latency, 5xx rate, HikariCP active connections, and database locks.
  </Step>
  <Step title="Break">
    Deploy an unindexed query and hold a database transaction open.
  </Step>
  <Step title="Observe">
    Follow the alert to metrics, traces, logs, and `pg_stat_activity`.
  </Step>
  <Step title="Fix">
    Roll back, cancel/terminate the blocker, and add the missing index concurrently.
  </Step>
  <Step title="Prove">
    Write a postmortem with timeline, 5 Whys, corrective actions, and prevention tests.
  </Step>
</Steps>
