...
- `@Configuration` contributes the executor bean to Spring.
- `@Bean(name = "taskExecutor")` defines the default async executor name Spring can use.
- `setCorePoolSize(8)` keeps a baseline of worker threads alive.
- `setMaxPoolSize(16)` limits burst concurrency so async work cannot create unlimited threads.
- `setQueueCapacity(500)` bounds memory used by queued tasks.
- `setThreadNamePrefix("async-exec-")` makes thread dumps searchable.
- `setTaskDecorator(new MdcTaskDecorator())` copies MDC from request thread to worker thread.
- `MDC.getCopyOfContextMap()` snapshots the caller's logging context before thread handoff.
- `MDC.setContextMap(contextMap)` restores it in the worker thread.
- `MDC.clear()` prevents correlation IDs from leaking into the next task on the same pooled thread.

Proof task:

```console
Set requestId in a servlet filter.
Call an @Async method that logs once.
Verify the async log line contains the same requestId.
Remove the TaskDecorator and verify the requestId disappears.
```
...
## Logging & Monitoring Fundamentals Interview Map

Logging and monitoring are not decorations. They are how production tells you what is happening.

### 1. Log level meaning

| Level | Use |
| :--- | :--- |
| `TRACE` | Extremely detailed local debugging |
...
Do not log every request body at `INFO`. Do not log secrets at any level.

### 2. Structured logs beat plain strings

Bad:

```java
log.info("order failed for user " + userId);
```

Better:

```java
log.warn("Order payment declined userId={} orderId={} reason={}",
        userId, orderId, reason);
```

Best production output is structured JSON with `traceId`, `spanId`, service, route, status, and safe domain identifiers.

### 3. What should trigger a page?
...
- Queue lag causing missed deadlines.
- Saturation of critical pools.

Do not page only because CPU is high. High CPU may mean the worker is doing useful work.

### 4. Error budget basics

If the SLO is `99.9%`, the allowed failure is `0.1%`.

Burn-rate alerts ask: "How fast are we spending that allowed failure?"

### Build-Break-Fix Lab

<Steps>
  <Step title="Build">
    Add structured JSON logs and RED metrics to a checkout endpoint.
  </Step>
  <Step title="Break">
    Log raw tokens, add `userId` as a metric label, and alert on average latency only.
  </Step>
  <Step title="Observe">
    Identify secret leakage, metric cardinality explosion, and hidden p99 pain.
  </Step>
  <Step title="Fix">
    Mask secrets, move high-cardinality values to logs/traces, and alert on SLO burn rate.
  </Step>
  <Step title="Prove">
    Add tests or log assertions that sensitive fields are masked and metrics expose only safe labels.
  </Step>
</Steps>
