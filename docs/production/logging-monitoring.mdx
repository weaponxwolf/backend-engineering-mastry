...
description: "Master backend observability: Logback async ring buffers, MDC context propagation across thread pools, structured JSON logs, PII masking, and Google SRE burn-rate alerts."
---
...
graph TD
    ...
    subgraph Logback Engine
        AsyncApp["AsyncAppender (Disruptor Ring Buffer / BlockingQueue)"]
...
<Tip>
**Production Rule**: Always wrap file and console appenders with **`ch.qos.logback.classic.AsyncAppender`**. Log events are dropped into a lock-free memory buffer, allowing application threads to return in sub-microsecond time.
</Tip>
...
<configuration scan="true" scanPeriod="30 seconds">
    ...
    <appender name="CONSOLE_JSON" class="ch.qos.logback.core.ConsoleAppender">
        <encoder class="net.logstash.logback.encoder.LogstashEncoder">
            ...
            <!-- Auto-include all MDC key-values -->
            <includeMdcKeyName>*</includeMdcKeyName>
            <timestampPattern>yyyy-MM-dd'T'HH:mm:ss.SSSZZ</timestampPattern>
        </encoder>
    </appender>

    <!-- Async Wrapper: Non-blocking logging buffer -->
    <appender name="ASYNC_JSON" class="ch.qos.logback.classic.AsyncAppender">
        <appender-ref ref="CONSOLE_JSON"/>
        <queueSize>2048</queueSize>
        <discardingThreshold>0</discardingThreshold> <!-- Never discard INFO/WARN under pressure -->
        <maxFlushTime>5000</maxFlushTime>
        <neverBlock>true</neverBlock> <!-- Drop TRACE/DEBUG if buffer saturates rather than block threads -->
    </appender>
