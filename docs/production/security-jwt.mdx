...
- `refresh_families.id` groups all rotated tokens from one login session.
- `user_id` ties the session family to a real account.
- `revoked` lets the server kill the entire session family after reuse detection.
- `expires_at` enforces an absolute session lifetime even if the user keeps refreshing.
- `refresh_tokens.token_hash` stores a hash, never the raw credential. A database leak should not hand attackers usable refresh tokens.
- `UNIQUE` on `token_hash` prevents duplicate token records.
- `family_id` connects each token to its family.
- `used` marks single-use consumption.
- `idx_refresh_tokens_family` makes family revocation and cleanup efficient.

Proof task:

```console
Login creates family F and token R1 hash.
Refresh with R1 marks R1 used and creates R2.
Refresh with R1 again revokes family F.
Refresh with R2 after family revocation returns 401.
Verify revocation is committed even though the HTTP response is an error.
```
...
## JWT & Session Fundamentals Interview Map

Token security is about storage, validation, rotation, and resource authorization.

### 1. Access token vs refresh token

| Token | Purpose | Lifetime | Storage |
| :--- | :--- | :--- | :--- |
| Access token | Call APIs | Short | memory or secure platform storage |
| Refresh token | Get new access tokens | Longer | `HttpOnly`, `Secure`, `SameSite` cookie or secure server-side session |

If an access token leaks, short expiry limits damage. If a refresh token leaks, rotation and reuse detection limit long-term takeover.

### 2. Stateless does not mean logout is solved

Pure JWT access tokens are valid until expiry. If you need immediate revocation, you need:

- Short access-token TTL.
- Refresh-token rotation.
- Server-side token family state.
- Optional denylist or introspection for high-risk scopes.

### 3. Claims are not trusted until verified

Bad:

```java
String role = parseJwtWithoutVerifying(token).getClaim("role");
```
...
```mermaid
flowchart LR
    Parse["Parse token"] --> Signature["Verify signature"]
    Signature --> Issuer["Validate issuer"]
    Issuer --> Audience["Validate audience"]
    Audience --> Time["Validate exp/nbf"]
    Time --> Claims["Map trusted claims"]
```

### 4. XSS vs CSRF

XSS lets attacker-controlled JavaScript run in your site. It can steal tokens from `localStorage`.

CSRF abuses browser auto-sent credentials like cookies. It cannot usually add custom bearer headers from another origin.

Token storage is a trade:

- `localStorage`: easier, but exposed to XSS.
- `HttpOnly` cookie: hidden from JavaScript, but needs CSRF defense.

### Build-Break-Fix Lab

<Steps>
  <Step title="Build">
    Implement login, access token validation, refresh token rotation, and protected note retrieval.
  </Step>
  <Step title="Break">
    Accept unsigned tokens, skip `aud`, store refresh token in localStorage, and check only roles.
  </Step>
  <Step title="Observe">
    Forge claims, reuse old refresh tokens, and access another user's resource.
  </Step>
  <Step title="Fix">
    Enforce issuer/audience/algorithm, rotate refresh tokens, and scope database queries by authenticated owner.
  </Step>
  <Step title="Prove">
    Add tests for expired token, wrong audience, refresh reuse, wrong owner, and CSRF-protected cookie endpoints.
  </Step>
</Steps>
