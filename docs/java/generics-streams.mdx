...
#### Production Fix: Push Sorting And Limiting To The Database

```java
public interface OrderRepository extends JpaRepository<Order, Long> {

    @Query("""
        select new com.example.OrderDTO(o.id, o.totalAmountCents, o.createdAt)
        from Order o
        where o.status = 'COMPLETED'
        order by o.totalAmountCents desc, o.id desc
        limit 10
        """)
    List<OrderDTO> findTop10CompletedOrders();
}
```

Line by line:

- `select new ... OrderDTO(...)` creates a DTO projection directly from selected columns. Hibernate does not need to materialize full tracked entities.
- `where o.status = 'COMPLETED'` filters in the database, close to indexes and disk pages.
- `order by o.totalAmountCents desc, o.id desc` lets PostgreSQL use an index order instead of sorting millions of Java objects in heap memory.
- `o.id desc` is a deterministic tie-breaker. Without it, equal totals can appear in unstable order across requests.
- `limit 10` means the database returns only the ten rows the API needs.
- `List<OrderDTO>` makes the method contract small and read-only.

Matching index:

```sql
CREATE INDEX idx_orders_completed_total_id
ON orders (total_amount_cents DESC, id DESC)
...
```console
Seed 2,000,000 orders.
Run EXPLAIN (ANALYZE, BUFFERS) for the SQL.
Verify the plan uses the index and returns 10 rows.
Run a heap profiler against the old stream version and the SQL version.
Explain why the SQL version moves memory pressure from JVM heap to database query planning/index access.
```
...
## Generics & Streams Fundamentals Interview Map

Generics protect types at compile time. Streams describe data pipelines. Both can hurt production when used blindly.
...
values.add("paid");
Integer status = (Integer) values.get(0); // runtime ClassCastException
```
...
The compiler blocks wrong types before production.

### 2. Why generic type information disappears

Java erases generic parameters for backward compatibility:
...
Both become mostly `List` at runtime. This is why you cannot write:

```java
if (value instanceof List<String>) { }
```

### 3. When to use `? extends` and `? super`
...
```java
void addIds(List<? super Long> output) { ... }
```

PECS: Producer Extends, Consumer Super.

### 4. Streams are not automatically faster
...
This is dangerous on huge data:
...
The database should filter, sort, and paginate using indexes. Do not pull millions of rows into heap and sort them in Java.

### 5. `parallelStream()` is not a web performance button

`parallelStream()` uses the shared `ForkJoinPool.commonPool()`. If one request blocks that pool with database or HTTP calls, other unrelated work in the JVM suffers.

Use virtual threads, dedicated executors, or database-side work instead.

### Build-Break-Fix Lab

<Steps>
  <Step title="Build">
    Implement a small order summarizer using typed collections and sequential streams.
  </Step>
  <Step title="Break">
    Introduce raw types, unchecked casts, `.sorted()` on a huge list, and `parallelStream()` around blocking I/O.
  </Step>
  <Step title="Observe">
    Capture `ClassCastException`, heap growth, and common-pool starvation.
  </Step>
  <Step title="Fix">
    Restore generic types, push sorting to SQL, and remove blocking parallel streams.
  </Step>
  <Step title="Prove">
    Add unit tests for type-safe transformations and a repository test proving SQL sorting/pagination.
  </Step>
</Steps>
