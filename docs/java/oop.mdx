...
#### Production Fix: Put The Invariant Inside The Object
...
public class ProductInventory {
    ...
    public void reserve(int quantity) {
        ...
        if (stockAvailable < quantity) {
            throw new InsufficientInventoryException(stockAvailable, quantity);
        }
...
- `private int stockAvailable` prevents outside code from changing stock without using the domain method.
- `private int reservedStock` protects the second half of the invariant. Available and reserved stock must move together.
- `reserve(int quantity)` names the business action. Callers do not say "set available"; they say "reserve."
- `quantity <= 0` rejects impossible commands before mutating state.
- `stockAvailable < quantity` checks the invariant at the point where the state transition happens.
- `stockAvailable -= quantity` and `reservedStock += quantity` happen together in the same method, so one cannot be forgotten by another service.
- The read methods expose state without exposing setters.

Proof test:

```console
Given stockAvailable=3 and reservedStock=0
When reserve(2)
Then stockAvailable=1 and reservedStock=2
When reserve(2) again
Then InsufficientInventoryException is thrown and both values remain unchanged
```

This does not replace database-level concurrency protection. It protects the in-memory domain transition. Under concurrent HTTP requests, you still need a transaction, row lock, optimistic version, or atomic SQL update.
