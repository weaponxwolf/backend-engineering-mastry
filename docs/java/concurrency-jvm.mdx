...
1. Atomicity: Can an operation be interrupted mid-execution?
   Example: count++ decomposes into 3 distinct CPU instructions (read, add, write).
...
### The Carrier Pinning Trap: The #1 Virtual Thread Disaster
...
| Mechanism             | Region-based evacuation     | Colored pointers & Load      |
|                       | with incremental pauses     | barriers (Fully concurrent)  |
| Throughput Overhead   | Low (~2-5% CPU overhead)    | Moderate (~5-10% CPU overhead)|
| Recommended Use Case  | Standard APIs, general pods | Low-latency APIs, finance,   |
|                       | with standard latency SLA   | large cache in-memory nodes  |
+-----------------------+-----------------------------+------------------------------+
```
...
<AccordionGroup>
  ...
  <Accordion title="2. Why doesn't the volatile keyword make counter++ thread-safe?">
    `counter++` is a compound read-modify-write operation consisting of three distinct bytecode instructions: `getfield`, `iadd`, and `putfield`. While `volatile` guarantees visibility (ensuring the `getfield` reads the most recent value from cache and `putfield` flushes immediately), it does not guarantee mutual exclusion. If two threads execute `counter++` concurrently, both can read the same initial value before either writes the incremented result, causing lost updates.
  </Accordion>
