...
### Loom's Solution: Scoped Values (JEP 446 / JEP 481)
Virtual threads maintain their own lightweight thread-local storage, but for maximum performance and immutability across child tasks, Java introduces `ScopedValue`:
```java
public final static ScopedValue<UserPrincipal> CURRENT_USER = ScopedValue.newInstance();

// Bound for the duration of the lambda, inherited by virtual child tasks, automatically cleaned up
...
## 6. The Modern Architectural Decision Rubric (2026+)
...
| **Relational DB Ecosystem** | Requires R2D2 (limited features, no Hibernate) | Full JPA / Hibernate 6, Liquibase, Flyway |
| **Third-Party Libraries** | Any blocking library poisons the EventLoop | Any blocking library works seamlessly |
| **Backpressure Support** | Native, fine-grained stream backpressure | TCP socket-level backpressure only |
...
### When to Choose Spring MVC + Virtual Threads (90% of Backend Applications):
...
3. **Engineering Team Productivity**: Teams that want clear stack traces, step-through breakpoints, and zero risk of freezing event loops.
