...
#### Production Fix: Move Blocking Work Off The EventLoop

```java
@PostMapping
public Mono<OrderResponse> submitOrder(@RequestBody OrderRequest request) {
    return Mono.fromCallable(() -> legacyDbClient.verifyMerchantBlocked(request.merchantId()))
...
- `Mono.fromCallable(...)` wraps the blocking JDBC call so it runs when subscribed, not during pipeline assembly.
- `legacyDbClient.verifyMerchantBlocked(...)` is still blocking; the code does not pretend otherwise.
- `subscribeOn(Schedulers.boundedElastic())` moves that blocking call onto Reactor's bounded elastic worker pool instead of Netty EventLoop threads.
- `flatMap(...)` continues the reactive chain after the blocking result is available.
- `Mono.error(...)` represents failure inside the stream instead of throwing directly on an EventLoop.
- `processOrderReactive(request)` should remain non-blocking if this endpoint stays WebFlux.

Proof task:

```console
Log Thread.currentThread().getName() inside the blocking call.
It should show boundedElastic, not reactor-http-nio.
Run a load test with one slow legacy DB call and verify /actuator/health still responds.
```

Senior caveat: this is a containment fix, not a reason to mix blocking calls everywhere in WebFlux. If most of the application is blocking JDBC and imperative service logic, Spring MVC with virtual threads may be simpler and safer.
...
#### Production Fix: Avoid Blocking I/O Inside `synchronized`
...
- `ReentrantLock` allows virtual threads to park without the same carrier pinning hazard as blocking inside `synchronized`.
- `lock.lock()` protects the critical section.
- The `try` block keeps the protected code visually small.
- `finally` guarantees unlock even when the socket call throws.

Better design:

```console
Do not hold any lock while doing network I/O.
Read or compute the local state needed for the call.
Release the lock.
Perform the socket call.
Reacquire a lock only if shared in-memory state must be updated.
```
...
Exercise the cache path under load.
Pinned-thread stack traces should disappear after removing synchronized blocking I/O.
```
