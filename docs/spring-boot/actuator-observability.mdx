...
description: "Zero assumptions guide to Spring Boot Actuator: The car dashboard analogy, /actuator/health setup, Kubernetes probes, Micrometer metrics, and thread dump diagnostics."
---

## 1. What is Spring Boot Actuator? (The Car Dashboard Analogy)

When you drive a car down the highway at 60 mph, you do not crawl under the hood with a wrench to check if the engine has enough oil or if the alternator is spinning.

You glance at the **car dashboard**:
- The **speedometer** tells you how fast you are moving.
- The **fuel gauge** tells you how much gas you have left.
- The **temperature gauge** warns you if the engine is overheating.
- The **check-engine light** warns you before the car completely breaks down.

```mermaid
flowchart LR
    subgraph CarAnalogy["The Car Dashboard Mental Model"]
        Engine["Car Engine / Transmission<br/>(Your Backend Code & Database)"] --> Dashboard["Dashboard Instrument Cluster<br/>(Spring Boot Actuator)"]
        Dashboard --> Driver["Driver / Mechanic<br/>(Engineer / Kubernetes)"]
    end
```

**Spring Boot Actuator is the instrument dashboard for your running application**:
- Instead of guessing whether your database is connected, you check `GET /actuator/health`.
- Instead of wondering if JVM memory is full, you check `GET /actuator/metrics/jvm.memory.used`.
- Instead of restarting your app to debug an issue, you change log levels live via `POST /actuator/loggers`.

---

## 2. Turning On Actuator in 30 Seconds (Your First Health Check)

To enable Actuator, add this single starter dependency to your `pom.xml`:
...
Now, boot your Spring Boot application and open your browser or terminal:
...
1. Spring Boot detected `spring-boot-starter-actuator` on your classpath.
2. It auto-configured the `/actuator/health` endpoint.
3. It pinged your application's internal state. Because your JVM is running and healthy, it returns `{"status": "UP"}`.
4. If you have a DataSource (PostgreSQL) configured, Actuator automatically runs a test query (like `SELECT 1`) against the database. If the database crashes, Actuator changes the status to `{"status": "DOWN"}` and returns HTTP `503 Service Unavailable`!

---

## 3. Physical Architecture: The Control Plane vs. The Data Plane
...
## 4. Kubernetes Probes: Liveness vs. Readiness vs. Startup
...
## 5. Writing Production Custom HealthIndicators
...
## 6. Production Metrics with Micrometer & Prometheus
...
## 7. Dynamic Runtime Logging (`/actuator/loggers`)
...
## 8. Thread Dumps & Heap Dumps: Diagnosing Production Freezes
...
## 9. Securing Actuator in Production
...
## 10. Active Recall & Interview Drills
