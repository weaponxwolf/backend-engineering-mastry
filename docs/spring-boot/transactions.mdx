...
1. **Hibernate Snapshot Bypass**: In a standard read-write transaction, Hibernate creates an in-memory snapshot copy of every loaded entity to detect mutations during dirty checking. Setting `readOnly = true` tells Hibernate to **skip snapshot creation**, reducing snapshot overhead for eligible entities. The memory saving depends on the workload; it is not a fixed 50%.
...
3. **Read-Replica Routing**: `readOnly = true` alone does not select a replica. You must implement routing, configure separate data sources, and ensure the route is selected before acquiring the connection. Replica lag also affects read-after-write behavior. See [Spring transaction settings](https://docs.spring.io/spring-framework/reference/data-access/transaction/declarative/annotations.html).
