...
- Spring IoC Container and Spring AOP proxy generation (CGLIB dynamic class proxies).
- JDBC connection pools (HikariCP) and thread-to-connection binding via `ThreadLocal`.
...
1. **Hibernate Snapshot Bypass**: In a standard read-write transaction, Hibernate creates an in-memory snapshot copy of every loaded entity to detect mutations during dirty checking. Setting `readOnly = true` tells Hibernate to **skip snapshot creation**, reducing snapshot overhead for eligible entities.
...
3. **Read-Replica Routing**: `readOnly = true` alone does not select a replica. You must implement dynamic data source routing with Spring's `AbstractRoutingDataSource`.

---

### 7.1 Dynamic Read/Write Replica Routing via `AbstractRoutingDataSource`

In production, enterprise architectures scale read throughput by separating a single **Primary PostgreSQL Node** (handling all inserts, updates, deletes) from multiple **Standby Read Replicas**:

```mermaid
flowchart TD
    Client[Incoming Request] --> Service[Spring Boot Service Layer]
    Service -->|@Transactional readOnly=false| Router[RoutingDataSource]
    Service -->|@Transactional readOnly=true| Router
    Router --> LazyProxy[LazyConnectionDataSourceProxy]
    LazyProxy -->|Writes / Mutations| Primary[(PostgreSQL Primary)]
    LazyProxy -->|Reads / Queries| Standby[(PostgreSQL Read Replicas)]
```

#### The LazyConnectionDataSourceProxy Requirement (The Critical Gotcha)

<Warning>
**The Connection Borrow Timing Trap**:
By default, Spring's `DataSourceTransactionManager` borrows a connection from the `DataSource` **before** the transaction interceptor determines whether the transaction is read-only!

Without wrapping your routing data source in a `LazyConnectionDataSourceProxy`, the routing logic executes when `TransactionSynchronizationManager.isCurrentTransactionReadOnly()` is still `false`. As a result, **every query gets sent to the Primary, completely defeating read replication!**

`LazyConnectionDataSourceProxy` defers actual connection acquisition from HikariCP until the first SQL statement is physically executed, ensuring the read-only flag is set.
</Warning>
...
public class DataSourceConfig {
    ...
    public DataSource dataSource(@Qualifier("routingDataSource") DataSource routingDataSource) {
        // Defers physical connection borrow until first statement execution
        return new LazyConnectionDataSourceProxy(routingDataSource);
    }
}
```

#### The Replication Lag "Read-Your-Own-Writes" Consistency Hazard

When a user writes data (e.g., submits a comment) and is immediately redirected to the view page, asynchronous PostgreSQL streaming replication may have a 50ms–500ms replication lag. If the view query routes to the replica, the user does not see their own newly created comment!

**Production Mitigation**: Implement a **Session Stickiness Window** (e.g., using a short-lived HTTP cookie or Redis key `user:{id}:last_write`). If a user has performed a mutation within the last 3 seconds, force all incoming reads for that user session to hit the **Primary** data source, guaranteeing immediate read-your-own-writes consistency.
...
| **Replica Inconsistency** | Query hits replica during replication lag | User sees stale data immediately after mutation | Use Session Stickiness or `LazyConnectionDataSourceProxy` routing rules. |

---

## 9. Hands-on Practice Challenge & Integration Tests
...
public class OrderService {
    ...
    public void processOrder(Order order) {
        // Calling annotated method internally via 'this'
        saveWithAudit(order);
    }
...
class OrderServiceTransactionTest {
    ...
    void internalCall_failsToRollback_corruptingDatabaseState() {
        ...
        // When invoking processOrder, the internal saveWithAudit throws RuntimeException
        assertThrows(RuntimeException.class, () -> orderService.processOrder(order));

        // FATAL: Due to proxy bypass, the order was committed to the DB despite the exception!
...
public class OrderService {
    ...
    public void processOrder(Order order) {
        // Enforces transaction boundary explicitly regardless of internal caller
        transactionTemplate.execute(status -> {
            orderRepository.save(order);
...
<AccordionGroup>
  ...
  <Accordion title="5. Why is LazyConnectionDataSourceProxy mandatory when configuring read/write replica routing?">
    Without `LazyConnectionDataSourceProxy`, Spring's `DataSourceTransactionManager` acquires a connection from the `DataSource` before method execution begins—at a point when `TransactionSynchronizationManager.isCurrentTransactionReadOnly()` is still `false`. Wrapping the routing data source in `LazyConnectionDataSourceProxy` defers actual connection acquisition from the pool until the first SQL statement is executed, by which time the transaction's read-only flag is active.
  </Accordion>
</AccordionGroup>
...
- [ ] Implement transparent Read/Write replica routing using `AbstractRoutingDataSource` and `LazyConnectionDataSourceProxy`.
</Check>
