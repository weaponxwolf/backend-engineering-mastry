...
### 1.1 The Everyday Analogy: The Corporate Office Turnstile & Keycard

To understand Spring Security without getting overwhelmed by filters and tokens, imagine entering a high-security corporate headquarters:

```mermaid
flowchart TD
    subgraph BuildingSecurity["Corporate Office Security Gate"]
        Visitor["Visitor arrives at revolving doors<br/>(Untrusted HTTP Request)"] --> Turnstile["Turnstile & Metal Detector<br/>(Spring Security Filter Chain)"]
        Turnstile -->|"Step 1: Scan Keycard<br/>'Who are you?'"| Auth["Authentication<br/>(JWT / Session / Password)"]
        Auth -->|"Step 2: Check Floor Access<br/>'Can you visit Floor 4?'"| Authz["Authorization<br/>(Role: ADMIN vs USER)"]
        Authz -->|"Step 3: Wear Badge on Lanyard"| Badge["SecurityContextHolder<br/>(ThreadLocal in RAM)"]
        Badge --> Elev["Take Elevator to Office<br/>(@RestController Method)"]
        Elev --> Leave["Visitor Leaves Building<br/>(Return HTTP Response)"]
        Leave --> Clean["Guard Revokes Pass!<br/>(ThreadLocal.clear())"]
    end

    style Turnstile fill:#ffebee,stroke:#c62828,stroke-width:2px;
    style Elev fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px;
```

### The Two Questions Every Security System Asks:
1. **Authentication ("Who are you?")**:
   - You show your passport, driver's license, or scan your employee badge.
   - In Spring Security, this is validating your username/password or verifying the digital cryptographic signature on a **JWT (JSON Web Token)**.
2. **Authorization ("What are you allowed to do?")**:
   - Just because you work at the company does not mean you can walk into the CEO's office or open the money vault!
   - In Spring Security, this is checking roles and permissions: `@PreAuthorize("hasRole('ADMIN')")` or checking if User #42 is allowed to view Invoice #108.

### The Fatal Identity Bleed: The Forgotten Visitor Badge
- When a Tomcat worker thread finishes serving your request, it returns to the thread pool to serve another customer 5 milliseconds later.
- If Spring Security forgets to wipe your badge from memory (`ThreadLocal.clear()`), the next visitor who steps into the elevator **inherits your CEO badge**!
- This is the #1 vulnerability in custom Spring Security filters: the **ThreadLocal Identity Bleed**.

---

## Phase 2: Physical First Principles

To master Spring Security, you must understand the hostile physical environment of network programming.

### 2.1 The Hostile Physical Network
...
- `@RequestMapping("/api/v1/users/{userId}/notes")` exposes `userId` in the path, which means it is client-controlled input.
- `@PreAuthorize("hasRole('ADMIN') or #userId == authentication.principal.id")` checks the path user ID before the controller method runs.
- `hasRole('ADMIN')` allows administrators through a separate role path.
- `#userId` refers to the method argument named `userId`.
- `authentication.principal.id` reads the authenticated user ID from the security context.
- `noteService.getNote(userId, noteId)` must still scope the database query by both `userId` and `noteId`. Method security is not a replacement for ownership-scoped queries.
- `@PostAuthorize(...)` checks the returned object after method execution. It is useful when ownership is not known until after loading, but it can still load data before denying access.

Proof tests:

```console
User 7 requests /api/v1/users/7/notes/10 -> allowed if note belongs to 7.
User 7 requests /api/v1/users/8/notes/10 -> 403 before service method runs.
ADMIN requests /api/v1/users/8/notes/10 -> allowed.
User 7 requests /api/v1/users/7/notes/999 owned by user 8 -> service/repository must still return 404 or 403.
```

Senior rule: route-level roles answer "is this kind of caller allowed here?" Ownership-scoped queries answer "is this exact caller allowed to touch this exact row?"
...
<AccordionGroup>
  ...
  <Accordion title="5. How does ThreadLocal state leakage happen in Spring Security, and how is it prevented?">
    Tomcat handles incoming requests using a reusable thread pool (e.g., 200 worker threads). By default, `SecurityContextHolder` stores security credentials in a `ThreadLocal` variable. If a request finishes without clearing the context, the thread returns to the pool carrying the previous user's credentials. When the thread is reused for an unauthenticated visitor, that visitor inherits the previous user's identity. Spring Security prevents this via `SecurityContextHolderFilter`, which ensures `SecurityContextHolder.clearContext()` is called inside a `finally` block at the end of every request.
  </Accordion>

  <Accordion title="6. Why does CORS preflight fail if security filters are misconfigured?">
    Before sending cross-origin mutating requests (like `POST` or `PUT` with JSON), browsers automatically send an HTTP `OPTIONS` preflight request without authentication headers. If the Spring Security filter chain requires authentication on all incoming requests before the `CorsFilter` can evaluate headers, the preflight request is rejected with `401 Unauthorized`. To prevent this, `CorsFilter` must be positioned before `BearerTokenAuthenticationFilter` or `UsernamePasswordAuthenticationFilter`, or configured via `http.cors()`.
  </Accordion>

  <Accordion title="7. What is the difference between @PreAuthorize and @PostAuthorize?">
    `@PreAuthorize` evaluates authorization expressions **before** the method is entered; if the check fails, the method never executes. `@PostAuthorize` executes the method first, captures the return value, and evaluates the expression with access to `returnObject` (e.g. `@PostAuthorize("returnObject.ownerId == authentication.principal.id")`). If the check fails, Spring throws an `AccessDeniedException`, preventing the return value from being delivered to the caller.
  </Accordion>

  <Accordion title="8. Why is BCrypt workload calibration critical for production stability?">
    BCrypt is intentionally CPU-intensive to thwart brute-force password cracking. The cost factor (log rounds) exponentially increases hashing time: cost 10 takes $\approx 80\text{ ms}$, cost 12 takes $\approx 300\text{ ms}$, and cost 14 takes $\approx 1.2\text{ seconds}$ per hash. If an attacker submits 50 concurrent login requests on an uncalibrated cost factor, all Tomcat CPU worker threads saturate at 100%, causing Kubernetes liveness probes to time out and restarting the container in a cascading crash loop.
  </Accordion>
</AccordionGroup>
...
<Check>
- [ ] Diagram the request flow from Tomcat $\rightarrow$ `DelegatingFilterProxy` $\rightarrow$ `FilterChainProxy` $\rightarrow$ `SecurityFilterChain`.
- [ ] Configure Spring Security 6 using modern functional Lambda DSL (`HttpSecurity.authorizeHttpRequests`).
- [ ] Implement stateless REST security: disabled CSRF, `SessionCreationPolicy.STATELESS`, and custom CORS.
- [ ] Understand the 15 standard filters in the default Spring Security filter chain and their ordering.
- [ ] Prevent ThreadLocal context leaks using `SecurityContextHolder.clearContext()` in `finally` blocks.
- [ ] Format 401 Unauthorized and 403 Forbidden error responses using RFC 9457 `ProblemDetail`.
- [ ] Protect against BOLA/IDOR using SpEL method security: `@PreAuthorize("#userId == authentication.principal.id")`.
- [ ] Differentiate `@PreAuthorize` (pre-execution check) from `@PostAuthorize` (post-execution returnObject check).
- [ ] Configure CORS correctly to handle browser preflight `OPTIONS` requests before authentication filters.
- [ ] Avoid wildcard origins (`*`) when `allowCredentials(true)` is set to comply with browser security rules.
- [ ] Calibrate BCrypt password encoder work factors (cost 10-12) to balance security with CPU starvation risks.
- [ ] Build custom `AuthenticationEntryPoint` and `AccessDeniedHandler` implementations.
- [ ] Differentiate cookie-based ambient credentials requiring CSRF from header-based bearer tokens.
- [ ] Write slice security tests with MockMvc verifying 401 and 403 responses using `@WithMockUser`.
- [ ] Verify that downstream threads (e.g. `@Async`) receive security context using `DelegatingSecurityContextAsyncTaskExecutor`.
</Check>
