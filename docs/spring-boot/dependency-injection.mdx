...
#### Production Fix: Constructor Injection With Final Dependencies
...
- `@Service` makes the class a Spring bean candidate during component scanning.
- `private final OrderRepository orderRepository` makes the dependency mandatory and impossible to replace after construction.
- `private final PaymentClient paymentClient` declares the external collaborator explicitly.
- The constructor is the only way to create a valid `OrderService`.
- Spring sees one constructor and automatically resolves both parameters from the application context.
- `processOrder(...)` uses dependencies that are guaranteed non-null after startup.
...
verify(paymentClient).authorize(order.paymentRequest());
```

This test uses plain Java construction. No Spring container, reflection, or database socket is required.
