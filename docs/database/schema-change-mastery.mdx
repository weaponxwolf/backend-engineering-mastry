...
- **PostgreSQL 11+**: A non-volatile default, such as `DEFAULT 'PENDING'`, usually avoids a table rewrite. A volatile expression such as `random()` still requires a rewrite. Both forms need an `ACCESS EXCLUSIVE` lock, so even a metadata-only change can wait behind other transactions. Use a short `lock_timeout` and retry deliberately. See [PostgreSQL column changes](https://www.postgresql.org/docs/16/ddl-alter.html).
...
1. **Phase 1**: Registers an invalid index in PostgreSQL system catalogs (`pg_index`). It becomes ready for write maintenance later in the build.
2. **Phase 2 (Table Scan 1)**: Scans all existing table pages to build the initial B-Tree. Reads and writes proceed normally!
3. **Phase 3 (Table Scan 2)**: Validates changes from concurrent writes. PostgreSQL then waits for relevant old snapshots before marking the index valid (`indisvalid = true`).

### 4.2 Flyway Multi-Statement Rule
`CREATE INDEX CONCURRENTLY` **cannot execute inside a multi-statement transaction block**. 

Keep the index build in its own migration. Configure that migration with a **sibling script configuration file**, whose name includes both `.sql` and `.conf`. A SQL comment named `flyway:transactional` is not a supported Flyway setting.

```sql
-- src/main/resources/db/migration/V5__add_orders_customer_idx.sql
CREATE INDEX CONCURRENTLY idx_orders_customer_date
ON orders (customer_id, created_at DESC);
```

```properties
# src/main/resources/db/migration/V5__add_orders_customer_idx.sql.conf
executeInTransaction=false
```

This is Flyway's documented [per-script configuration mechanism](https://documentation.red-gate.com/flyway/reference/script-configuration). For Flyway versions using PostgreSQL transactional advisory locks, configure the migration runner to use session locks:

```properties
# flyway.conf for the standalone Flyway runner
flyway.postgresql.transactional.lock=false
```

This is a separate setting from migration transaction boundaries. See [Flyway's PostgreSQL lock setting](https://documentation.red-gate.com/flyway/reference/configuration/flyway-namespace/flyway-postgresql-namespace/flyway-postgresql-transactional-lock-setting). Verify both settings using the exact Flyway version and runner you deploy; a standalone `flyway.conf` is not automatically Spring Boot configuration.

`CONCURRENTLY` permits normal writes, but still takes locks, waits for transactions, and consumes CPU and I/O. A failed build can leave an invalid index. Avoid `IF NOT EXISTS` as a repair mechanism: an existing index with the same name might be invalid or have a different definition. Inspect it before retrying. See [PostgreSQL concurrent index rules](https://www.postgresql.org/docs/16/sql-createindex.html).
...
<AccordionGroup>
  ...
  <Accordion title="3. Why can CREATE INDEX CONCURRENTLY not run inside a Flyway transaction?">
    ...
    PostgreSQL explicitly forbids running `CREATE INDEX CONCURRENTLY` inside an existing user transaction block (`BEGIN ... COMMIT`). To run it via Flyway, put `executeInTransaction=false` in the migration’s sibling `.sql.conf` file and configure PostgreSQL session advisory locks when required by the Flyway runner.
  </Accordion>
...
<Check>
All large table indexing uses `CREATE INDEX CONCURRENTLY` with `executeInTransaction=false` in the migration’s sibling `.sql.conf` file.
</Check>
<Check>
