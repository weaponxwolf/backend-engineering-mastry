...
```sql
SELECT customer_id, SUM(total_cents) AS total_spent_cents
FROM orders
WHERE total_spent_cents > 100000
GROUP BY customer_id;
```

`WHERE total_spent_cents > 100000` fails because `total_spent_cents` does not exist yet at the `WHERE` step.

Proof task:

```console
Run both queries against a small orders table.
Observe the alias failure.
Move the aggregate filter to HAVING.
Explain why WHERE filters rows and HAVING filters groups.
```
