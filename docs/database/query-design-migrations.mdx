...
```properties
# src/main/resources/db/migration/V4__create_orders_created_at_idx.sql.conf
...
| **Concurrent Index Deadlock** | `CREATE INDEX CONCURRENTLY` without non-transactional directive | Flyway fails with `cannot run inside transaction block` | Add a sibling `.sql.conf` with `executeInTransaction=false`; check PostgreSQL advisory-lock mode. |
...
- [ ] Execute non-blocking index creation using a sibling `.sql.conf` with `executeInTransaction=false` and `CREATE INDEX CONCURRENTLY`.
