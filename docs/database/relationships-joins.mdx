...
- The foreign key column lives in the child table.
- Deletes, ownership checks, joins, and anti-joins all need to find child rows by `user_id`.
- Without this index, PostgreSQL may scan the whole child table while holding locks, blocking writes and melting connection pools.

Proof task:

```console
Run EXPLAIN (ANALYZE, BUFFERS) for the NOT EXISTS query before adding idx_orders_user_id.
Create the index.
Run the same EXPLAIN again.
Confirm the child lookup becomes indexed and buffer reads drop.
```
...
Relational modeling is not drawing boxes. It is choosing which facts must be protected by the database.

### 1. Primary Key vs Foreign Key

A primary key identifies one row. A foreign key points to a row in another table.
...
The foreign key is a physical constraint. It prevents an order from referencing a user that does not exist.
...
Many posts can have many tags, so use a join table:
...
Do not store comma-separated IDs in one column. That destroys referential integrity, indexing, joins, and query correctness.

### 3. Normalization vs denormalization

Normalize data when you need one authoritative fact:

```console
users.email lives in users, not copied into every order
```

Denormalize when you need historical snapshots or faster read models:

```console
order_items.product_name_snapshot preserves what the buyer saw at purchase time
```

**Senior answer:** "Normalization protects write correctness. Denormalization is a deliberate read or history optimization, and it needs an ownership and update strategy."

### 4. Delete behavior is a business decision
...
Never add cascade because it is convenient. Cascading from `users` to `orders` could erase financial history.

### Build-Break-Fix Lab

<Steps>
  <Step title="Build">
    Model users, posts, comments, tags, and post_tags with constraints.
  </Step>
  <Step title="Break">
    Store tag IDs as comma-separated text and delete a user with historical orders.
  </Step>
  <Step title="Observe">
    Querying becomes awkward, constraints disappear, or important records vanish.
  </Step>
  <Step title="Fix">
    Use join tables, correct foreign keys, and deliberate delete behavior.
  </Step>
  <Step title="Prove">
    Add tests for impossible references, duplicate join rows, and protected deletes.
  </Step>
</Steps>
