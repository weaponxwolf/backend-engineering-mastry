...
## 7. The "Distributed Monolith" Anti-Pattern
...
```text
The 4 Hallmarks of a Distributed Monolith:
1. Shared Database:
   Multiple microservices connect to the exact same PostgreSQL schema.
   Consequence: A schema migration in Service A breaks Service B in production.

2. Deep Synchronous Call Chains:
   Service A calls Service B -> calls Service C -> calls Service D.
   Consequence: Latency is additive, and availability is multiplicative. If one service hiccups, all upstream callers crash.

3. Distributed Transactions (2PC):
   Attempting cross-service ACID locks across network boundaries.
   Consequence: HikariCP connection pool exhaustion and deadlocks.

4. Lockstep Deployments:
   Service A cannot be deployed unless Service B and C are deployed simultaneously.
   Consequence: Destroys independent team agility.
```
...
<AccordionGroup>
  ...
  <Accordion title="3. How does the Transactional Outbox pattern eliminate the Dual-Write problem?">
  ...
  <Accordion title="4. How do you solve cross-service queries in microservices without performing cross-database SQL joins?">
    Cross-database joins are solved using **CQRS (Command Query Responsibility Segregation)** with read projections:
    
    Instead of joining tables across the network via HTTP, read-optimized services subscribe to domain events published by write services. The read service denormalizes and pre-aggregates the data into an optimized read store (PostgreSQL materialized view, Elasticsearch, or MongoDB). Queries are resolved with a fast, single-table fetch.
  </Accordion>

  <Accordion title="5. What is the structure of a W3C traceparent header, and why is it essential in microservices?">
    The W3C `traceparent` header is formatted as `version-traceid-parentid-traceflags` (e.g. `00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01`). 
    - `traceid`: Identifies the entire distributed transaction across all microservice hops.
    - `parentid`: Identifies the immediate caller (span) that dispatched the request.
    
    It is essential because it allows monitoring systems (Zipkin, Jaeger, Tempo) to reconstruct the full end-to-end distributed waterfall of an operation across disparate HTTP calls, message queues, and database executions.
  </Accordion>
</AccordionGroup>
...
<Check>
W3C `traceparent` distributed trace headers are propagated across all HTTP and Kafka network boundaries.
</Check>
