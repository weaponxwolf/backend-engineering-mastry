...
- `@EnableWebSocketMessageBroker` switches on Spring's STOMP broker pipeline. Without it, the endpoint may exist but messages will not route through STOMP destinations.
- `setApplicationDestinationPrefixes("/app")` reserves `/app/...` for messages that should hit server-side `@MessageMapping` handlers.
- `enableSimpleBroker("/topic", "/queue")` creates an in-memory broker for local fan-out. It is fine for one pod; multiple pods need a shared backplane or external broker.
- `setHeartbeatValue(new long[]{25000, 25000})` sends periodic heartbeats in both directions. This helps detect dead mobile connections that never sent a clean close frame.
- `addEndpoint("/ws/connect")` is the HTTP upgrade URL. Before it becomes a WebSocket, it is still an HTTP request with cookies, headers, and origin checks.
- `setAllowedOriginPatterns("https://*.production-domain.com")` blocks arbitrary websites from opening authenticated sockets to your backend.
- `.withSockJS()` adds fallback transports for clients that cannot use native WebSockets, but those fallbacks have different scaling behavior.
- `setDisconnectDelay(30000)` gives unstable clients a grace window before server-side session cleanup.

Proof drill: connect two browser clients to different pods and publish through `/app`. If only one pod's local clients receive the message, the Redis backplane is missing or miswired.
...
Redis bridge reading:

- `StringRedisTemplate` publishes cluster-wide messages to Redis channels.
- `SimpMessagingTemplate` sends messages only to clients connected to the current JVM pod.
- `publishToCluster(...)` is used when a local client sends a message that every pod must learn about.
- `convertAndSend(destinationTopic, json)` makes Redis the cross-pod fan-out path. Redis Pub/Sub is ephemeral; if a pod is offline, it misses the message.
- `onMessage(...)` runs when Redis delivers a broadcast to this pod.
- `message.getChannel()` becomes the STOMP destination, such as `/topic/room.42`.
- `localMessagingTemplate.convertAndSend(topic, payload)` is the final hop from this pod to its own connected sockets.

What breaks without this bridge: a user connected to Pod A sends a chat message, but a recipient connected to Pod B never sees it because WebSocket connections are pinned to physical pods.
...
## Realtime Fundamentals Interview Map

Realtime does not always mean WebSockets. Choose the transport from the direction, frequency, and delivery needs.

### 1. Polling vs SSE vs WebSocket

| Transport | Direction | Best For |
| :--- | :--- | :--- |
| Polling | Client asks repeatedly | Simple low-frequency refresh |
| SSE | Server to client | Notifications, LLM token stream, stock ticker |
| WebSocket | Both directions | Chat, multiplayer, collaborative editing |

WebSockets add stateful connection management. Do not choose them just because they sound advanced.

### 2. A WebSocket is still a TCP connection

After HTTP upgrade, the server keeps a socket open. That consumes:
...
### 3. Horizontal scaling problem

If User A connects to Pod 1 and User B connects to Pod 4, Pod 1 cannot write directly to B's socket. The socket lives in Pod 4 memory.

Use a message backplane:

```mermaid
flowchart LR
    Pod1["Pod 1<br/>Alice socket"] --> Redis["Redis/Kafka backplane"]
    Redis --> Pod4["Pod 4<br/>Bob socket"]
```

### 4. Delivery guarantees matter

Redis Pub/Sub is fast but at-most-once. A disconnected pod misses messages.

Kafka is durable and replayable but has higher operational complexity and latency.

Choose based on whether missed events are acceptable.

### Build-Break-Fix Lab

<Steps>
  <Step title="Build">
    Implement a small chat or notification stream with two application pods.
  </Step>
  <Step title="Break">
    Store sessions only in a local map and send messages across pods.
  </Step>
  <Step title="Observe">
    Messages disappear when sender and recipient are connected to different pods.
  </Step>
  <Step title="Fix">
    Add a Redis/Kafka backplane and heartbeat-based connection cleanup.
  </Step>
  <Step title="Prove">
    Run two pods, connect users to different pods, and verify delivery plus cleanup after forced disconnect.
  </Step>
</Steps>
