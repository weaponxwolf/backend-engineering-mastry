...
- `@Scheduled(cron = "0 0 2 * * *")` fires inside every pod running this code. Kubernetes does not know that the method should run only once globally.
- `@SchedulerLock` wraps the method with a database-backed lock attempt before business code runs.
- `name = "NightlyBillingJob"` must be stable and unique. Changing this string creates a different lock and can accidentally let the old and new jobs overlap during deploy.
- `lockAtMostFor` is crash recovery. It must be longer than the worst credible job duration, or a second pod can start while the first pod is still working.
- `lockAtLeastFor` is duplicate prevention under fast jobs and small clock differences. It should be long enough to absorb skew, not so long that it blocks legitimate reruns.
- `billingService.chargeCustomerSubscriptions()` must still be idempotent. ShedLock reduces duplicate starts; it does not protect against manual reruns, retries after partial failure, or lock expiry during a long job.

Proof drill: run two app instances against the same database with the same clock schedule. Exactly one instance should log the billing execution, and the other should log lock denial or no-op behavior.
...
- `name` is the logical job key and the primary key. The database uses that uniqueness to prevent two pods from owning the same job at once.
- `lock_until` is the expiry timestamp checked by competing pods.
- `locked_at` is audit evidence for incident debugging.
- `locked_by` should contain pod or hostname identity so operators can find the owner during a stuck job.
...
## Batch & Scheduling Fundamentals Interview Map

Batch jobs are production systems. They need locking, restartability, backpressure, and auditability.

### 1. Why `@Scheduled` duplicates work in Kubernetes

Every pod has its own JVM and Spring scheduler.

```mermaid
flowchart LR
    Cron["00:00"] --> Pod1["Pod 1 runs job"]
    Cron --> Pod2["Pod 2 runs job"]
    Cron --> Pod3["Pod 3 runs job"]
```

Without a distributed lock, all replicas run the job.

### 2. At-most-once vs at-least-once

| Goal | Meaning | Risk |
| :--- | :--- | :--- |
| At-most-once | Job never runs twice | It may not run if worker dies |
| At-least-once | Job eventually runs | It may run more than once |
| Exactly-once effect | Repeated execution has one business effect | Requires idempotent writes |

Most production jobs aim for at-least-once execution with idempotent effects.

### 3. Chunk size is a trade-off
...
### 4. Restartability requires metadata

A restartable job records progress after each committed chunk. Logs alone are not enough.

### Build-Break-Fix Lab

<Steps>
  <Step title="Build">
    Create a scheduled invoice job that processes rows in chunks.
  </Step>
  <Step title="Break">
    Run 3 replicas without ShedLock and crash during chunk 5.
  </Step>
  <Step title="Observe">
    See duplicate execution or lost progress.
  </Step>
  <Step title="Fix">
    Add ShedLock, idempotency keys, Spring Batch metadata, and restartable chunk processing.
  </Step>
  <Step title="Prove">
    Start multiple pods and verify one job owner; kill the owner and verify safe restart.
  </Step>
</Steps>
